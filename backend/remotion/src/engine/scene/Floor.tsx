...
export const Surface: React.FC<{
  ...
  size: number;
  /** Centre tint, usually a very dark brand colour. */
  color: string;
  ...
  <Placed x={0} y={y} z={0} rotateX={90} width={size} height={size}>
    <div
      style={{
        ...
        inset: 0,
        opacity,
        // Fades to transparent, not black: the plane's far edge would
        // otherwise draw a hard horizon across the environment.
        background: `radial-gradient(ellipse at 50% ${focus * 100}%, ${color} 0%, rgba(0,0,0,0.7) 32%, rgba(0,0,0,0.35) 48%, transparent 62%)`,
      }}
    />
  </Placed>
);
